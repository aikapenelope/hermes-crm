<script lang="ts">
	...
	const KANBAN_COLS: { id: string; label: string; accent: string }[] = [
		...
		{ id: 'in_progress', label: 'EN PROGRESO', accent: 'border-amber-500/40' },
		{ id: 'done',        label: 'COMPLETADO',  accent: 'border-emerald-500/40' },
...
<div class="flex-1 p-5 md:p-6 uppercase tracking-widest" style="font-size:11px;">
	...
	<div class="mb-5 flex items-center justify-between">
		<div>
			...
			<p class="mt-0.5 text-[10px] text-[#555]">
				{viewMode === 'list' ? (result ? `${result.totalItems} tareas` : '…') : `${allTasks.length} tareas`}
			</p>
		</div>
	...
	{#if viewMode === 'list'}
		...
		{:else if result && result.items.length === 0}
			<Empty
				title="Sin tareas"
				description={filterStatus || filterPriority ? 'Prueba con otro filtro' : 'Crea tu primera tarea'}
			>
				...
				{#each result.items as task (task.id)}
					<div
						class="group flex items-center gap-3 border border-[#1a1a1a] bg-[#090909] px-4 py-3
							...
							{['done','cancelled'].includes(task.status) ? 'opacity-50' : ''}"
					>
						<!-- Checkbox -->
						<button
						...
						<button onclick={() => openEdit(task)} class="flex-1 min-w-0 text-left">
							...
							{#if task.expand?.contact}
								<a href="/contacts/{task.expand.contact.id}"
						...
						<div class="flex shrink-0 items-center gap-2">
							...
							<div class="flex gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
								...
								<button onclick={() => { deleteTarget = task; }}
							...
							{#each col.tasks as task (task.id)}
								<div
									class="group border border-[#1a1a1a] bg-[#0a0a0a] p-3 transition-colors hover:border-[#333]
										{movingId === task.id ? 'opacity-40' : ''}"
								>
									<!-- Card top -->
									<div class="flex items-start justify-between gap-2 mb-2">
										<button onclick={() => openEdit(task)} class="text-left flex-1 min-w-0">
										...
										<div class="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
											<button onclick={() => openEdit(task)} class="p-1 text-[#444] hover:text-white">
												<Pencil class="h-2.5 w-2.5" />
											</button>
											<button onclick={() => { deleteTarget = task; }} class="p-1 text-[#444] hover:text-red-400">
										...
										<div class="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
											{#if statusPrev[task.status]}
												<button
													...
													class="border border-[#222] px-1.5 py-0.5 text-[7px] tracking-widest
														text-[#555] uppercase hover:border-[#444] hover:text-white transition-colors"
												>←</button>
											{/if}
											{#if statusNext[task.status]}
												<button
													...
													class="border border-[#222] px-1.5 py-0.5 text-[7px] tracking-widest
														text-[#555] uppercase hover:border-white hover:text-white transition-colors"
												>→</button>
											{/if}
										</div>
									</div>
...
>
	{#snippet children()}
		<TaskForm task={editingTask} onSave={handleSaved} onClose={() => { sheetOpen = false; }} />
	{/snippet}
</Sheet>
