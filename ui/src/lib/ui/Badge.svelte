<script lang="ts">
	interface Props {
		variant?: 'slate' | 'blue' | 'green' | 'amber' | 'red' | 'orange' | 'violet';
		size?: 'sm' | 'md';
	...
	// All on dark #070707 bg — no fills, border-only for legibility
	const styles: Record<string, string> = {
		slate:  'border border-[#333] text-[#888]',
		blue:   'border border-white text-white',
...
<span class="inline-flex items-center uppercase font-medium {styles[variant]} {sizes[size]}">
	{@render children()}
</span>
